	...
	SubShader
	{
		Pass
		{
			...
			vertexOutput vert(vertexInput input)
			{
                ...
                // output.pos.z += sin(_WaveSpeed*windSample*_Time.x)*_WaveAmp * heightFactor;
                // output.pos.x += cos(_WaveSpeed*windSample*_Time.x)*_WaveAmp * heightFactor;

                float4 vertex = input.vertex;

                vertex.x *= cos(20 * _Time.x); //*_WaveAmp * heightFactor;
                vertex.z *= cos(20 * _Time.x); //*_WaveAmp * heightFactor;
